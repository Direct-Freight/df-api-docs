...
var createHar = function (swagger, path, method, queryParamValues) {
  ...
  var baseUrl = getBaseUrl(swagger)
    ...
    /* JG what really needs to be done is to detect the {} in the path, look up that element in the swagger
     * via swagger.paths[path][method].parameters and then select the first item in the enum.
     */

 var realpath = path

if (typeof swagger.paths[path].parameters !== 'undefined') {
        console.log(swagger.paths[path].parameters)
	...
	realpath = realpath.replace(new RegExp('{' + param.name + '}', 'g'), 'foundit');
      }
    }
  }
